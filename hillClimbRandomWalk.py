# -*- coding: utf-8 -*-
"""
Created on Mon Apr 11 22:48:06 2022
...
probCheck =0.45    #with probability p
random_walk =0      #flag to know if random walk is initiated 
...
while done == 0:
    ...
    random_walk =0      #flag to know if random walk is initiated 
    if probRand > probCheck:    #with probability 1-p, it will perform a random walk
        while random_walk == 0:
            solutionsChecked += 1
            x_curr = myPRNG.choice(Neighborhood)    #choose random neighbor
            if evaluate(x_curr)[0] >= 0:            #evaluate the best soltuion
                x_best = x_curr[:]
                f_curr = evaluate(x_curr)
                f_best = f_curr
                random_walk = 1
                best_value.append(f_best)
    else:                                   #with probability P, it will perform normal hill climbing
        for s in Neighborhood:
            ...
            if evaluate(s)[0] > f_best[0]:
                ...
                f_best = evaluate(s)[:]
                best_value.append(f_best)
        if solutionsChecked >= 50000:          #stopping criterion
            done = 1
        else:
            x_curr = x_best[:]
            f_curr = f_best[:]
